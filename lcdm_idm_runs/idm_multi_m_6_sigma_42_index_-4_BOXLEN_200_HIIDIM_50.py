...
NN = 5
sigma0 = np.linspace(-42,-41, NN)

for i in range(len(sigma0)):
    cosmo_params['sigma_SDM'] = sigma0[i] 
    wr_path = '/scratch1/rahimieh/21cmfirstclass_outputs/idm_multi_m_6_sigma_%.2f_index_-4_BOXLEN_200_HIIDIM_50/' %sigma0[i]
    print("idm run %.0f starts" %i)
    lightcone_SDM = p21c.run_lightcone(redshift = 6., random_seed = 1,regenerate = True, write = True,direc= wr_path,user_params = user_params,astro_params = astro_params,flag_options = flag_options,cosmo_params = cosmo_params,global_quantities = global_quantities,lightcone_quantities = lightcone_quantities)
