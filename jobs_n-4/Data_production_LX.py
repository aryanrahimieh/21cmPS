...
subdir_small_data = r"./outputs_astro_LX_BOXLEN_%1.0f_HIIDIM_%1.0f/" %(user_params['BOX_LEN'], user_params['HII_DIM'])

if not os.path.exists(subdir_small_data):
    os.makedirs(subdir_small_data)

for astr_val in astr_vals:
    wr_path = subdir_org_data + r'lcdm_LX_%1.6f_BOXLEN_%1.0f_HIIDIM_%1.0f/' %(astr_val, user_params['BOX_LEN'], user_params['HII_DIM'])
    print("\n", wr_path)
    ...
    global_temp_filename = subdir_small_data + r"Tb_LX_%1.6f.csv" %astr_val
    ...
    ps_filename = subdir_small_data +  r"ps_LX_%1.6f.npz" %astr_val
    ...
    input_filename = subdir_small_data +  r"input_LX_%1.6f.json" %astr_val
