...
#################functions########################################
def run_lcone_lcdm(seed, wr_path, user_params, astro_params, flag_options, cosmo_params, global_quantities, lightcone_quantities):
    start_time = time.time()
    lightcone = p21c.run_lightcone(redshift = 6., # minimu
                                    random_seed = seed, # numerical seed -- 
...
##########running and savinglightcones for diff seeds#########
seed_vals = np.arange(3,11)
print(seed_vals)
...
subdir_small_data = r"./outputs_lcdm_BOXLEN_%1.0f_HIIDIM_%1.0f/" %(user_params['BOX_LEN'], user_params['HII_DIM'])

if not os.path.exists(subdir_small_data):
    os.makedirs(subdir_small_data)

for seed in seed_vals:
    wr_path = subdir_org_data + r'lcdm_seed_%1.0f_BOXLEN_%1.0f_HIIDIM_%1.0f/' %(seed, user_params['BOX_LEN'], user_params['HII_DIM'])
    print("\n", wr_path)
    ### 1-running lightcones
    lightcones[wr_path] = run_lcone_lcdm(seed, wr_path, 
    ...
    global_temp_filename = subdir_small_data + r"Tb_lcdm_seed_%1.0f.csv" %seed 
    ...
    ps_filename = subdir_small_data +  r"ps_lcdm_seed_%1.0f.npz" %seed 
    ...
    input_filename = subdir_small_data +  r"input_lcdm_seed_%1.0f.json" %seed 
